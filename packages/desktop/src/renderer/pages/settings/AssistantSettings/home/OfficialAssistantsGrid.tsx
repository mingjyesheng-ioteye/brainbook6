...
import { BRAINBOOK_BRAND, isBrainbookAssistantId, partitionBrainbookAssets } from '@/renderer/brainbook/brand';
...
const OfficialAssistantsGrid: React.FC<OfficialAssistantsGridProps> = ({
  ...
  const assistantGroups = useMemo(
    () => partitionBrainbookAssets(officialAssistants, (assistant) => assistant.id, isBrainbookAssistantId),
    [officialAssistants]
  );
  ...
  const renderAssistantCards = (items: AssistantListItem[]) =>
    items.map((assistant) => {
