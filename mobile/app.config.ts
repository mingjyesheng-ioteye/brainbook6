...
export default ({ config }: ConfigContext): ExpoConfig => {
  return {
    ...config,
    name: 'BrainBook',
    slug: 'brainbook-mobile',
    version: VERSION.version,
    orientation: 'portrait',
    icon: './assets/images/icon.png',
    scheme: 'brainbook-mobile',
    userInterfaceStyle: 'automatic',
    ios: {
      supportsTablet: true,
      bundleIdentifier: 'ai.resopod.brainbook',
      buildNumber: String(VERSION.buildNumber),
      infoPlist: {
        ITSAppUsesNonExemptEncryption: false,
        NSCameraUsageDescription: 'BrainBook needs camera access to scan QR codes for server login.',
      },
    },
    android: {
      adaptiveIcon: {
